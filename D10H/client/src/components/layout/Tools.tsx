...
const Tools: React.FC<IToolsProps> = () => {
  ...
  const { setCountdown, setMetronome, setSolo, setMute } =
    usePlayScoreDispatch();
  ...
  const handleTunner: () => void = useCallback(() => {}, [onOpen]);
  ...
  const handleLearningModeOn: () => void = useCallback(() => {}, [onOpen]);

  const handleLooperOn: () => void = useCallback(() => {}, [onOpen]);

  const handleSolo: () => void = useCallback(() => {
    if (solo) {
      setSolo(false);
      if (mute) {
        setMute((prev) => [{ ...prev[0], muted: false }, ...prev.slice(1)]);
      }
    } else {
      setSolo(true);
      if (mute) {
        setMute((prev) => [{ ...prev[0], muted: true }, ...prev.slice(1)]);
      }
    }
  }, [solo, mute, setSolo, setMute]);

  const handleMuteOn: () => void = useCallback(() => {}, [onOpen]);
    ...
    (): ToolType[] => [
      ...
      },
      {
        ...
        content: (
          <CountdownIcon
            size="24px"
            color={countdown ? "#ad47ff" : "currentColor"}
          />
        ),
      ...
      },
      {
        ...
        content: (
          <MetronomeIcon
            size="24px"
            color={metronome ? "#ad47ff" : "currentColor"}
          />
        ),
      ...
      },
      {
        ...
        content: (
          <TempoIcon
            size="24px"
            color={
              tempoPercent < 100 || tempoPercent > 100
                ? "#ad47ff"
                : "currentColor"
            }
          />
      ...
      },
      {
        ...
        content: (
          <LearningModeIcon
            size="24px"
            color={
              learningMode.startTempoPercent === learningMode.endTempoPercent
                ? "currentColor"
                : "#ad47ff"
            }
          />
      ...
      },
      {
        ...
        content: (
          <LooperIcon
            size="24px"
            color={
              looper.startMeasure === 0 || looper.endMeasure === 0
                ? "currentColor"
                : "#ad47ff"
            }
          />
      ...
      },
      {
        name: "solo",
        content: (
          <SoloIcon size="24px" color={solo ? "#ad47ff" : "currentColor"} />
        ),
      ...
      },
      {
        ...
        content: (
          <MuteIcon
            size="24px"
            color={
              !solo && mute?.some((im) => im.muted) ? "#ad47ff" : "currentColor"
            }
          />
        ),
