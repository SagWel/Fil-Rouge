...
const PageAccountDisplay : React.FC<IPageAccountDisplayProps> = () => {
    ...
    const [notSuggestExplicitContent, setNotSuggestExplicitContent] = useState<boolean>(user?.isChildAccount as boolean || user?.filterExplicit === 'not_suggested' || user?.filterExplicit === 'hidden')
    const [hiddenExplicitContent, setHiddenExplicitContent] = useState<boolean>(user?.isChildAccount as boolean || user?.filterExplicit === 'hidden')
    ...
    const submitSuggestExplicitContent = async (filtered: boolean) => {
        ...
        if (!notSuggestExplicitContent && (hiddenExplicitContent || user?.filterExplicit === 'hidden')) {
            try {
                const res1: Response = await fetch(`http://${host}:${port}${urlFetchFilterExplicit}${user?.id}`, {
                method: 'POST',
                ...
                console.error("Erreur lors du changement des préférences d'affichage des contenus explicit", error)
            }

        } else {
            try {
                const res2: Response = await fetch(`http://${host}:${port}${urlFetchFilterExplicit}${user?.id}`, {
                method: 'POST',
                headers : {'Content-Type': 'application/json'},
                body: JSON.stringify ({
    ...
    const submitHiddenExplicitContent = async (hiddenned: boolean) => {
        try {
            const res: Response = await fetch(`http://${host}:${port}${urlFetchFilterExplicit}${user?.id}`, {
                method: 'POST',
                headers : {'Content-Type': 'application/json'},
                body: JSON.stringify ({
                ...
                borderRadius={'0.5rem'} border={'1px solid #4e4c51'}>
                    <Box>
                        ...
                        mt={'25px'}>
                            <FormControl w={'100%'} pos={'relative'}>
                                <Flex align={'center'}>
                                    ...
                                    <Switch isChecked={notSuggestExplicitContent}
                                    display={'inline-block'}
                                    pos={'relative'}
                            ...
                            mt={'25px'}>
                                <FormControl w={'100%'} pos={'relative'}>
                                    <Flex align={'center'}>
                                        ...
                                        <Switch isChecked={hiddenExplicitContent}
                                        display={'inline-block'}
                                        pos={'relative'}
                                        verticalAlign={'middle'} lineHeight={0}
                                        onChange={(e) => {
                                    ...
                                    fontSize={'0.75rem'}
                                    lineHeight={'normal'} color={'#a19fa4'}>
                                        Les contenus explicites ne seront pas recommandés, ne pourront pas être joués ni recherchés.
                                    </FormHelperText>
                                </FormControl>
                            </Box>
                        }
                        ...
                        }
                        {
                            ...
                            overflow={'hidden'}>
                                <chakra.span display={'inherit'}>
                                    <InfosIcon color="#E53E3E" lineHeight={'1em'} flexShrink={0} verticalAlign={'middle'} display={'block'} />
                                </chakra.span>
                                <Text
                                fontSize={'14px'} fontWeight={'400'} fontFamily={'Inter,Arial,sans-serif'}
                                lineHeight={'20px'} textDecor={'none'} color={'#E53E3E'}>
                                    Le changement de vos préférences n'as pas eu lieu. Merci de contacter le service client.
                                </Text>
                            </Flex>
                        }
                    </Box>
